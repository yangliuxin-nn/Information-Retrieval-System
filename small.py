...
"""
    This file is for the small corpus
...
def indexing(dirs, stopwords):
    ...
    # indexes: the key is the term, value is the a dictionary
    ...
    # store the index file called small.json
    tf = open("small.json", "w")
...
def load_relevance():
    ...
    # All documents in the small corpus are judged.
...
def bm25_doc(qry_terms, small_index):
    """
        ...
        :param small_index: the dictionary index file for the small corpus
...
def process_standard_queries(stopwords, small_index):
    """
    ...
    with open('files/queries.txt', 'r') as f:
        with open('output.txt', 'w', encoding='utf-8') as fWrite:
            for line in f:
                ...
                sims = bm25_doc(qry_terms, small_index)
                ...
                Though other designs may perform better on the small corpus, I can only choose one to be consistent with the large corpus.
                '''
...
def manual(stopwords, small_index):
    """
...
def evaluation(results, qrels, n):
    ...
    for j in results:
        ...
        count_non = 0
        bpref = 0
        # For small corpus, the number of relevant is the len(qrels[j])
        len_qrels = len(qrels[j])  # Important to modify for large corpus!!!
        ...
        for i in range(0, len(results[j])):
            ...
            if doc in qrels[j]:
                ...
                # the number of returned documents is larger than n, which is 10 for P@10
                # the reason for (i+1) rather than i is that i is the index number, (i+1) is the rank
                if (i + 1) <= n:
                    count_p_at_n += 1
                # if i < n, also update the values for R-precision
                # Adding the following if condition is to deal with the situation that:
                # the number of returned documents is larger than the number of relevant documents
                # the reason for (i+1) rather than i is that i is the index number, (i+1) is the rank
                if (i + 1) <= len(qrels[j]):
                    count_r_pre += 1
        ...
        recall_value += count / (len(qrels[j]))
        ...
        r_pre_value += count_r_pre / len(qrels[j])

        # AP of MAP
        map_value += p / len(qrels[j])
        ...
        # if the number of returned documents: p < the number of relevant: q < 10
        # if the number of returned documents: 10 > p > the number of relevant: q
    ...
    print("NDCG:".ljust(12), avg_NDCG10)  # try NDCG@10 but if len(results) < 10, it will not return NDCG@10
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Sorry, please follow the format 'python small.py -m [option]'")
        sys.exit()

    else:
        # further check whether the option is correct; if not, give hints and exit
        if option not in options:
            print("Sorry, please enter 'manual' or 'evaluation' after '-m'.")
            sys.exit()
        ...
        # if there is not an index file, create it at first
            ...
            indexing(os.listdir("documents"), stopwords)

            # end time
            end_time = time.process_time()

            # Preprocessing time is 0.609375 seconds
        ...
        start_time = time.process_time()
        tf = open("small.json", "r")
        small_index = json.load(tf)
        end_time = time.process_time()
        ...
        if option == "evaluation":
            ...
            results = process_standard_queries(stopwords, small_index)
            end_time = time.process_time()
            # Process standard queries takes 0.09375 second
            print("Processing standard queries takes {} seconds".format(end_time - start_time))
            ...
            # perform evaluation and print the metric scores
            evaluation(results, qrels, 10)

            # end time
            end_time = time.process_time()

            # Evaluation time is 0.0 second
            print("Evaluation time is {} seconds".format(end_time - start_time))

        # manual mode
        if option == 'manual':
            manual(stopwords, small_index)
